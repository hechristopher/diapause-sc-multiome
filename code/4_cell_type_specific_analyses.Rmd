...
mbhb_dia_DE <- compute_DE(mbhb_dia, mbhb_dev)
...
upset <- list(
  ...
  mbhb = mbhb_dia_DE[mbhb_dia_DE$DE == "Dia Up", "gene"],
...
## Compute Global Dia/Dev Peaks

```{r}

```

## Compute Cell Type Dia/Dev Peaks
...
mbhb_dia_motifs <- compute_motif_enrichment(mbhb_dia, mbhb_dev)
...
p_max <- 0.05
hm <- motif_master %>% filter(`dia vs dev p.adjust` < p_max |
                                `neuron dia p.adjust` < p_max | 
                                `brain dia p.adjust` < p_max |
                                `somite dia p.adjust` < p_max |
                                `epidermis dia p.adjust` < p_max)

enrich_cols <- c("dia vs dev fold.enrichment", "neuron dia fold.enrichment", 
                 "brain dia fold.enrichment", "somite dia fold.enrichment", 
                 "epidermis dia fold.enrichment")

pval_cols <- sub("fold.enrichment", "p.adjust", enrich_cols)


hm <- hm %>% mutate(
    across(all_of(enrich_cols), 
           ~ ifelse(get(sub("fold.enrichment", "p.adjust", cur_column())) > p_max, 0, .))
)
rownames(hm) <- hm$motif.name
hm <- hm[,enrich_cols]
hm <- hm %>% arrange(desc(across(colnames(hm))))
...
ht <- Heatmap(hm,
        ...
        column_labels = c("global", "neuron", "brain", "somite", "epidermis"),
