...
saveRDS(colors, paste0(outs_dir, "/colors_annotation_broad"))
...
all_peaks <- merge(all_peaks, closest_genes[,c("gene_name", "distance")], by = "row.names")
...
top.da.peak <- rownames(da_dia_dev[da_dia_dev$p_val_adj < 0.05 & da_dia_dev$avg_log2FC > 0.5, ])
...
enriched.motifs <- enriched.motifs[,c(8,4,5,6,9)]

colnames(enriched.motifs)[-1] <- paste0("dia vs dev ", colnames(enriched.motifs)[-1])
```
...
motif_master <- merge(enriched.motifs, diff.motifs, by = "row.names", all = T)
