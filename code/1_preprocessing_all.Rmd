...
meta$orig.ident <- factor(meta$orig.ident, 
                          levels = c("dev1d_1", "dev1d_2", "dev1d_3","hb_1", 
                                     "dia1d_1", "dia2d_1", "dia6d_1", "dia6d_2"))
...
SaveFigure(plot, "qc_vln_rna", width = 12, height = 6)
...
SaveFigure(plot, "qc_vln_atac", width = 12, height = 6)
...
```{r}
p_devtraj <- DimPlot(so_unfiltered, group.by = "devtraj",
        cols = c("#FBA91A", "#2C6E49" , "cadetblue1"),  label = TRUE)
p_orig.ident <- DimPlot(so_unfiltered, group.by = "orig.ident",
        cols = c("#FBA91A", "#FBA91A", "#FBA91A",
                 "cadetblue1", "#A8D5BA", "#7FC7B9", "#2C6E49", "#2C6E49"))
p_scDblFinder <- DimPlot(so_unfiltered, group.by = "scDblFinder.class.gex")
p_clusters <- DimPlot(so_unfiltered)

p <- ggarrange(p_devtraj, p_orig.ident, p_scDblFinder, p_clusters, ncol = 2, nrow = 2)
SaveFigure(p, "unfiltered_umaps", width = 20, height = 10)
...
so_filtered_medium$orig.ident <- factor(so_filtered_medium$orig.ident, 
                          levels = c("dev1d_1", "dev1d_2", "dev1d_3","hb_1", 
                                     "dia1d_1", "dia2d_1", "dia6d_1", "dia6d_2"))
...
```{r}
p_devtraj <- DimPlot(so_filtered_medium, group.by = "devtraj",
        cols = c("#FBA91A", "#2C6E49" , "#800080"),  label = TRUE)
p_orig.ident <- DimPlot(so_filtered_medium, group.by = "orig.ident",
        cols = c("#FBA91A", "#FBA91A", "#FBA91A",
                 "#800080", "#A8D5BA", "#7FC7B9", "#2C6E49", "#2C6E49"))
p_scDblFinder <- DimPlot(so_filtered_medium, group.by = "scDblFinder.class.gex")
p_clusters <- DimPlot(so_filtered_medium)
...
p <- DimPlot(pseudo, reduction = "pca",
             cols = c("#FBA91A", "#FBA91A", "#FBA91A","#800080", "#A8D5BA", "#7FC7B9", "#2C6E49", "#2C6E49"),
             label = T, repel = T, pt.size = 4)

SaveFigure(p, "psuedobulk_PCA", width = 5, height = 3)
```
...
density_plot <- ggplot(dat, aes(x=UMAP1, y=UMAP2)) +
  ...
  facet_wrap(~Sample, nrow=2, ncol=4) +
  theme_classic() +
...
```{r}
so_dev1d <- subset(x = so_filtered_medium, orig.ident %in% c("dev1d_1", "dev1d_2", "dev1d_3"))
so_dia1d <- subset(x = so_filtered_medium, orig.ident %in% c("dia1d_1"))
so_dia2d <- subset(x = so_filtered_medium, orig.ident %in% c("dia2d_1"))
so_dia6d <- subset(x = so_filtered_medium, orig.ident %in% c("dia6d_1", "dia6d_2"))
so_hb <- subset(x = so_filtered_medium, orig.ident %in% c("hb_1"))
```

```{r}
so_dev1d <- runSeuratRNAProcessing(so_dev1d)
so_dia1d <- runSeuratRNAProcessing(so_dia1d)
so_dia2d <- runSeuratRNAProcessing(so_dia2d)
so_dia6d <- runSeuratRNAProcessing(so_dia6d)
so_hb <- runSeuratRNAProcessing(so_hb)

```

```{r}
p1 <- DimPlot(so_dev1d) + ggtitle("dev1d")
p2 <- DimPlot(so_dia1d) + ggtitle("dia1d")
p3 <- DimPlot(so_dia2d) + ggtitle("dia2d")
p4 <- DimPlot(so_dia6d) + ggtitle("dia6d")
p5 <- DimPlot(so_hb) + ggtitle("hb")

p <- ggarrange(p1, p2, p3, p4, p5, ncol = 3, nrow = 2)
SaveFigure(p, name = "umaps_bysample", width = 15, height = 10)
```

```{r}
so_dia <- subset(x = so_filtered_medium,
                 orig.ident %in% c("dia1d_1", "dia2d_1", "dia6d_1", "dia6d_1"))

so_1d <- subset(x = so_filtered_medium,
                 orig.ident %in% c("dia1d_1", "dev1d_1", "dev1d_2", "dev1d_3"))

so_1d_hb <- subset(x = so_filtered_medium,
                 orig.ident %in% c("dia1d_1", "dev1d_1",
                                   "dev1d_2", "dev1d_3", "hb_1"))

```

```{r}
so_dia <- runSeuratRNAProcessing(so_dia)
so_1d <- runSeuratRNAProcessing(so_1d)
so_1d_hb <- runSeuratRNAProcessing(so_1d_hb)
```

```{r}
p1 <- DimPlot(so_dia, group.by = "orig.ident", cols = c("#A8D5BA", "#7FC7B9", "#2C6E49", "#2C6E49")) + 
  ggtitle("all diapause samples")
p2 <- DimPlot(so_1d, group.by = "orig.ident", cols = c("#FBA91A", "#FBA91A", "#FBA91A", "#A8D5BA")) + 
  ggtitle("dia1d and dev1d",)
p3 <- DimPlot(so_1d_hb, group.by = "orig.ident", cols = c("#FBA91A", "#FBA91A", "#FBA91A","#800080", "#A8D5BA")) +
  ggtitle("hb, dia1d, and dev1d")

p <- ggarrange(p1, p2, p3, ncol = 3, nrow = 1)
SaveFigure(p, name = "umaps_bysample_combination", width = 15, height = 5)
```
...
so_filtered_medium$timepoint <- factor(meta$timepoint, levels = c("hb", "dev1d", "dia1d", "dia2d", "dia6d"))
```

```{r}
timepoints = unique(so_filtered_medium$timepoint)
colors = c("hb" = "#800080", "dev1d" = "#FBA91A", "dia1d" = "#A8D5BA", "dia2d" = "#3E8E7E", "dia6d" = "#23583A")
...
# so_filtered_medium <- readRDS(paste0(outs_dir, "so_filtered_medium.rds"))
```

```{r}
saveRDS(so_filtered, paste0(outs_dir, "/so_final_filtered.rds"))
```
