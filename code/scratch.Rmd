...
```{r}
DefaultAssay(so) <- "RNA"
VlnPlot(so, features = c("jade2", "LOC107392522", "rbm5", "LOC107373896"), group.by = "diadev", pt.size = 0,
        cols = c("#FD8D3C", "#6BAED6"), ncol = 4)

FeaturePlot(so, features = "elavl3", reduction = "umap") + NoAxes() + NoLegend()
```

#Translate TFs from utoronto
```{r}
hum_tfs <- read.table("C:\\Users\\Christopher He\\OneDrive\\UCSF\\Singh Lab\\Genome Resources\\utoronto_human_TFs.txt")

nfur_tfs <- translate(hum_tfs$V1, from = "Human", multiple = "all", unlist_result = T)

writeLines(nfur_tfs, "C:\\Users\\Christopher He\\OneDrive\\UCSF\\Singh Lab\\Genome Resources\\nfur_utoronto_translated_TFs.txt")
```
...
```{r}
DefaultAssay(so) <- "RNA"
p <- FeaturePlot(so, reduction = "umap", features = c("LOC107387288", "LOC107376365")) + 
SaveFigure(p, path = "mapt_ctsd", type = "png", width = 8, height = 4)
```
```{r}
#so_neu <- readRDS("../results_all/lineage_subsets_old/so_neurons.rds")
DefaultAssay(so_neu) <- "RNA"
p <- FeaturePlot(so_neu, reduction = "umap.rna", features = c("LOC107387288", "LOC107376365"))
SaveFigure(p, path = "mapt_ctsd_neurononly", type = "png", width = 8, height = 4)
