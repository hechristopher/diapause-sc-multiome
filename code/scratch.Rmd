...
filtered_master <- master_genes[genes$Gene,]


filtered_master

ge <- FetchData(so[["RNA"]], vars = (genes %>% filter(Count_in_intersects > 1))$Gene)

ge$ct <- so$annotation_broad
ge$diadev <- ifelse(so$timepoint_group %in% c("earlydia", "middia", "latedia"), "dia", "dev")

#filter cts with less than 100 cells
cts <- rownames(table(ge$ct, ge$diadev))[apply(table(ge$ct, ge$diadev), 1, function(row) all(row > 100))]
ge <- ge %>% filter(ct %in% cts)

avg_exp_diadev_celltype <- ge %>%
  mutate(across(where(is.numeric), ~(.x - mean(.x))/sd(.x))) %>%
  group_by(ct, diadev) %>% 
  summarize_all(mean) %>%
  ungroup() %>%
  pivot_wider(names_from = diadev, values_from = where(is.numeric))

pct_expressed <- ge %>%
  group_by(ct, diadev) %>%
  summarize_all(~ sum(.x != 0)/n()) %>%
  ungroup() %>%
  pivot_wider(names_from = diadev, values_from = where(is.numeric))
...
Heatmap(as.matrix(pct_expressed %>% select(contains("dia"))), cluster_rows = F, cluster_columns = F, row_labels = avg_exp_diadev_celltype$ct)

Heatmap(as.matrix(pct_expressed %>% select(contains("dev"))), cluster_rows = F, cluster_columns = F, row_labels = avg_exp_diadev_celltype$ct)

```
#On disk storage and Sketch data
```{r}
so <- SketchData(so, ncells = 10000, method = "LeverageScore", sketched.assay = "sketch")
